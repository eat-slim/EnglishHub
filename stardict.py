...
class DictMySQL (object):
    ...
    # 注册新单词
    def register (self, word, items, commit = True):
        sql = 'INSERT INTO dictionary(word, sw) VALUES(%s, %s);'
        try:
            with self.__conn as c:
        ...
        if len(names) == 0:
            if commit:
                try:
                    ...
                    return False
            return False
        sql = 'UPDATE dictionary SET ' + ', '.join(['%s=%%s'%n for n in names])
        if isinstance(key, str) or isinstance(key, unicode):
            sql += ' WHERE word=%s;'
        else:
            sql += ' WHERE id=%s;'
...
if __name__ == '__main__':
    ...
    # connection = sqlite3.connect('ecdict.db')
    # c = connection.cursor()
    # c.execute('select* from stardict')
    # record = c.fetchone()
    # print(record)

    sd = StarDict('ecdict.db')
    print(sd.query('cocaine'))
